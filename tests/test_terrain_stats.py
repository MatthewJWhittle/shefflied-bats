...
from sdm.raster.terrain import (
    ...
    process_dem_to_terrain_attributes,
    save_terrain_dataset,
)
...
def test_calculate_slope(sample_dem_array):
    slope = calculate_slope(sample_dem_array, cell_size=1.0)
...
def test_calculate_aspect_components(sample_dem_array):
    aspect = calculate_aspect(sample_dem_array, cell_size=1.0)
...
def test_aspect_orientation_and_flats():
    ...
    aspect = calculate_aspect(dem, 1.0)
    inner = aspect[5:-5, 5:-5]
    assert np.allclose(float(inner.mean()), math.pi, atol=1e-5)
    flat = np.full((17, 17), 100.0, dtype=np.float32)
    asp_flat = calculate_aspect(flat, 10.0)
    assert np.isnan(asp_flat).all()

def test_calculate_twi(sample_dem_array):
    slope = calculate_slope(sample_dem_array, 1.0)  # arg kept for API, not used
    twi = calculate_twi(sample_dem_array, slope, cell_size=1.0)
...
def test_calculate_curvature(sample_dem_array):
    curvature = calculate_curvature(sample_dem_array, cell_size=1.0)
...
def test_curvature_laplacian_zero_on_plane():
    ...
    curv = calculate_curvature(dem, 5.0)
    inner = curv[5:-5, 5:-5]
    assert np.allclose(float(np.nanmax(np.abs(inner))), 0.0, atol=1e-6)

def test_roughness(sample_dem_dataset):
    # slope-like field
    sample_dem_dataset["slope"] = (("y", "x"), np.random.random((5, 5)))
    roughness = calculate_roughness(sample_dem_dataset.slope, window_size=3)
...
def test_calculate_tpi(sample_dem_dataset):
    tpi = calculate_tpi(sample_dem_dataset.dem, window_size=3)
...
def test_weighted_aspect(sample_dem_dataset):
    sample_dem_dataset["slope"] = (("y", "x"), np.random.random((5, 5)))
    aspect = calculate_aspect(sample_dem_dataset.dem.values, cell_size=1.0)
    e, n = calculate_aspect_components(aspect)
    sample_dem_dataset["aspect_eastness"] = (("y", "x"), e)
    sample_dem_dataset["aspect_northness"] = (("y", "x"), n)
    we, wn = calculate_weighted_aspect(
...
def test_nan_propagation_in_metrics():
    ...
    slope = calculate_slope(dem, 1.0)
    aspect = calculate_aspect(dem, 1.0)
    twi = calculate_twi(dem, slope, 1.0)
...
def test_process_terrain_stats_io_roundtrip(tmp_path):
    ...
    ds = process_dem_to_terrain_attributes(
        dem_path=in_tif,
...
def test_save_terrain_stats(tmp_path):
    ...
    ds = process_dem_to_terrain_attributes(in_tif, output_slope_units="radians")
    out_tif = tmp_path / "terrain.tif"
    path = save_terrain_dataset(ds, out_tif, drop_dem_variable=True)
    assert path.exists()
